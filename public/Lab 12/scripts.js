...
function notNum (symbols) {
	...
	for (var i = 0; i < array.length; i++) {
		if (numbers.indexOf(array[i]) == -1) { return true }
	}
}

function testName (name) {
	if (name == '') { alert('No input!') } else if (banned(name) == true) { alert('Number consist!') } else {
...
function testEmail (email) {
	...
	if (array.indexOf('@') < 1 || array.indexOf('.') == -1 || array.indexOf('.') - array.indexOf('@') < 2 || array.indexOf('.') == array.length - 1) { alert('Email is incorrect!') } else { alert('Email is correct') }
}
